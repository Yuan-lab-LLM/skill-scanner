...
class SkillScanner:
    ...
    def _scan_single_skill(self, skill: Skill, skill_directory: Path) -> ScanResult:
        ...
        skill_load_findings = self._skill_load_warning_findings(skill)
        ...
        try:
            ...
            all_findings.extend(skill_load_findings)
            analyzer_names: list[str] = []
            analyzers_executed: list[str] = []
            analyzers_requested = [analyzer.get_name() for analyzer in self.analyzers if analyzer.get_name() != "meta_analyzer"]
            ...
            for analyzer in self.analyzers:
                ...
                try:
                    ...
                    analyzers_executed.append(analyzer.get_name())
                except Exception as exc:
                    logger.exception("Analyzer %s failed for %s", analyzer.get_name(), skill.name)
                    analyzers_failed.append({"analyzer": analyzer.get_name(), "error": str(exc)})
                    errors.append(f"{analyzer.get_name()} failed: {exc}")
                    continue
            ...
            if llm_analyzers:
                ...
                for analyzer in llm_analyzers:
                    ...
                    if hasattr(analyzer, "last_error") and analyzer.last_error:
                        ...
                        errors.append(f"{analyzer.get_name()} failed: {analyzer.last_error}")
                        warnings.append(f"{analyzer.get_name()} did not complete successfully")
    ...
    def _skill_load_warning_findings(self, skill: Skill) -> list[Finding]:
        """Convert lenient loader warnings into explicit findings."""
        findings: list[Finding] = []
        for warning in getattr(skill, "load_warnings", []) or []:
            severity = Severity.MEDIUM
            rule_id = "SKILL_LOAD_WARNING"
            title = "Skill manifest loaded with warnings"
            remediation = "Fix SKILL.md so the scanner does not need to synthesize or backfill manifest fields."

            if "missing required field: name" in warning:
                severity = Severity.HIGH
                rule_id = "SKILL_MANIFEST_MISSING_NAME"
                title = "SKILL.md missing required field: name"
                remediation = "Add a stable, explicit 'name' field to SKILL.md frontmatter."
            elif "missing required field: description" in warning:
                severity = Severity.MEDIUM
                rule_id = "SKILL_MANIFEST_MISSING_DESCRIPTION"
                title = "SKILL.md missing required field: description"
                remediation = "Add a clear 'description' field to SKILL.md frontmatter."
            elif "Failed to parse YAML frontmatter" in warning:
                severity = Severity.HIGH
                rule_id = "SKILL_MANIFEST_INVALID_YAML"
                title = "SKILL.md frontmatter is malformed"
                remediation = "Fix the YAML frontmatter in SKILL.md so manifest fields can be parsed reliably."

            findings.append(
                Finding(
                    id=f"{rule_id}_{skill.name}",
                    rule_id=rule_id,
