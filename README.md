...
> **Fork Notice:** This repository is a fork of [cisco-ai-defense/skill-scanner](https://github.com/cisco-ai-defense/skill-scanner) and includes local adaptations for ClawManager integration and deployment workflows. Thanks to the Cisco AI Defense team for building and open-sourcing the original project.
...
- **ClawManager Ready** - This fork ships as a container image and includes `/scan-upload` compatibility, analyzer execution details, lenient upload handling, and local OpenAI-compatible LLM support

**[Join the Cisco AI Discord](https://discord.com/invite/nKWtDcXxtx)** to discuss, share feedback, or connect with the team.

---

## Fork Notes for ClawManager

This fork is maintained as a deployable security scanning component for ClawManager. In addition to upstream scanner capabilities, it includes:

- `/scan-upload` support for `use_llm`, `use_behavioral`, `enable_meta`, and `lenient`
- machine-readable analyzer execution fields such as `analyzers_requested`, `analyzers_executed`, `analyzers_failed`, `warnings`, and `errors`
- lenient upload handling so malformed `SKILL.md` files can still produce findings instead of only hard failures
- local OpenAI-compatible LLM support via `SKILL_SCANNER_LLM_BASE_URL` and raw model names such as `kimi-k2.5`
- corresponding meta-analyzer support via `SKILL_SCANNER_META_LLM_BASE_URL`

For ClawManager deployments, this repository's primary output is a container image.
...
Typical runtime environment variables for local OpenAI-compatible backends:

```bash
export SKILL_SCANNER_LLM_API_KEY="your_api_key"
export SKILL_SCANNER_LLM_MODEL="kimi-k2.5"
export SKILL_SCANNER_LLM_BASE_URL="http://your-openai-compatible-endpoint/v1"

export SKILL_SCANNER_META_LLM_API_KEY="your_api_key"
export SKILL_SCANNER_META_LLM_MODEL="kimi-k2.5"
export SKILL_SCANNER_META_LLM_BASE_URL="http://your-openai-compatible-endpoint/v1"
```

When `SKILL_SCANNER_LLM_BASE_URL` or `SKILL_SCANNER_META_LLM_BASE_URL` is set, this fork accepts raw model names without requiring an `openai/` prefix.

### ClawManager Service Integration

Expose the API server on port `8000`:

```bash
skill-scanner-api --host 0.0.0.0 --port 8000
```

Then point ClawManager to the service with:

```bash
SKILL_SCANNER_ENABLED=true
SKILL_SCANNER_BASE_URL=http://skill-scanner.<namespace>.svc.cluster.local:8000
SKILL_SCANNER_TIMEOUT_SECONDS=120
```
